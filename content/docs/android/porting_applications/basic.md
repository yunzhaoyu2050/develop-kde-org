...
For the app to successfully run on android, the `main` function needs to be exported. to do this, add a `Q_DECL_EXPORT` to the function header. Since it's only required on android, an `#ifdef` so that it is only added on android.
